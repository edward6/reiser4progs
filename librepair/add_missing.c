    ...
    reiser4_item_hint_t *hint, void *data) 
{
    ...
    items = reiser4_node_count(coord->node);
    units = reiser4_item_count(coord);
	...
	if (coord->pos.item != 0) {
		...
		neigh.pos.item = reiser4_node_count(neigh.node);
		neigh.pos.unit = ~0ul;
	    }
	}
...
{   
    ...
    if (hint->plugin->h.sign.group == NODEPTR_ITEM) {
	ptr = (reiser4_ptr_hint_t *)hint->hint;
	aux_bitmap_mark(am->bm_used, ptr->ptr);
    } else if (hint->plugin->h.sign.group == EXTENT_ITEM) {
	/* Extent item. */
...
errno_t repair_am_pass(repair_data_t *rd) {
	...
	if (level == TWIG_LEVEL) {
	    ...
	    for (pos->item = 0; pos->item < reiser4_node_count(node); 
		pos->item++) 
	    {
		...
		res = reiser4_tree_insert(tree, &hint, TWIG_LEVEL, NULL);
		
		if (res < 0) {
		    aal_exception_bug("Add missing pass failed to attach the "
			"extent item (%llu, pos %u) to the tree.", 
			blk, pos->item);
		    goto error_free_node;
		} else if (res > 0) {
		    /* Uninsertable. */
	    ...
	    coord.pos.item = reiser4_node_count(node) - 1;
	    coord.pos.unit = ~0ul;
	    ...
	    res = reiser4_tree_attach(tree, node, TWIG_LEVEL);
