...
reiser4_joint_t *repair_joint_open(reiser4_format_t *format, blk_t blk) {
...
static errno_t repair_joint_items_check(reiser4_joint_t *joint, 
    aux_bitmap_t *bm_used) 
{
    ...
    aal_assert("vpf-229", joint != NULL, return -1);
    aal_assert("vpf-517", joint->node != NULL, return -1);
    aal_assert("vpf-230", joint->node->entity != NULL, return -1);
    aal_assert("vpf-231", joint->node->entity->plugin != NULL, return -1);
...
/* 
    ...
    supported? 
*/
errno_t repair_joint_dkeys_check(reiser4_joint_t *joint, repair_data_t *data) {
    reiser4_coord_t coord;
...
static errno_t repair_joint_keys_check(reiser4_joint_t *joint) {
    reiser4_key_t key, prev_key;
    reiser4_pos_t pos = {0, ~0ul};
    errno_t res;
    
    aal_assert("vpf-258", joint != NULL, return -1);
...
*/
errno_t repair_joint_check(reiser4_joint_t *joint, aux_bitmap_t *bm_used) {
    int res;
    ...
    if ((res = repair_joint_items_check(joint, bm_used)))
	return res;
    
    if ((res = repair_joint_keys_check(joint)))
	return res;
