...
errno_t repair_ts_pass(repair_data_t *rd) {
	...
	if ((joint = repair_joint_open(rd->format, blk)) == NULL) {
	...
	aal_assert("vpf-544", plugin_call(goto error_free_joint, 
	    entity->plugin->node_ops, get_level, entity) == TWIG_LEVEL, 
	    goto error_free_joint);

	/* Lookup the joint. */	
	if ((res = reiser4_joint_traverse(joint, &hint, NULL, NULL,
...
static errno_t repair_ts_ovrl_add(reiser4_coord_t *coord, repair_data_t *rd) {
    ...
    if (!(joint = reiser4_coord_joint(coord))) {
	aal_exception_fatal("Failed to get the joint from the coord, but "
	    "it is expected that the coord is build on the joint.");
	return -1;
    }
    ...
    ovrl->joint = reiser4_coord_joint(coord);
    ovrl->pos = coord->pos;	
    ovrl->conflicts = 0;
