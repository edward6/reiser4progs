...
extern node_t *repair_node_open(reiser4_tree_t *tree, blk_t blk, bool_t check);

extern errno_t repair_node_check_struct(node_t *node, uint8_t mode);

extern errno_t repair_node_traverse(node_t *node, node_func_t func, void *data);

extern errno_t repair_node_clear_flags(node_t *node);

extern errno_t repair_node_pack(node_t *node, aal_stream_t *stream, int mode);
