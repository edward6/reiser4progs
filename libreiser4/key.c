	...
	void *data)			    /* item ot be checked */
{
	...
	return plugin_call(plugin->key_ops, confirm,
			   (key_entity_t *)data);
}

errno_t reiser4_key_guess(reiser4_key_t *key) {
	aal_assert("umka-907", key != NULL, return -1);
	...
	reiser4_key_t *src)	    /* source key */
{
	...
	aal_assert("umka-1114", src->plugin != NULL, return -1);

	dst->plugin = src->plugin;
	
	return plugin_call(src->plugin->key_ops, assign, dst, src);
}
	...
	reiser4_key_t *key)	    /* key to be clean */
{
	...
	plugin_call(key->plugin->key_ops, clean, key);
} 
	...
	const char *name)	    /* entry name to be hashed */
{
	...
	return plugin_call(key->plugin->key_ops, build_entry, key,
	...
	uint32_t type)		    /* new key type */
{
	...
	plugin_call(key->plugin->key_ops, set_type, key, type);
	...
	roid_t locality)	    /* new locality */
{
	...
	plugin_call(key->plugin->key_ops, set_locality, key, locality);
...
uint32_t reiser4_key_get_type(reiser4_key_t *key) {
	...
	return plugin_call(key->plugin->key_ops, get_type, key);
}
...
roid_t reiser4_key_get_objectid(reiser4_key_t *key) {
	...
	return plugin_call(key->plugin->key_ops, get_objectid, key);
}
...
void reiser4_key_maximal(reiser4_key_t *key) {
	...
	entity = plugin_call(key->plugin->key_ops, maximal,);
	aal_memcpy(key, entity, sizeof(*entity));
}
...
void reiser4_key_minimal(reiser4_key_t *key) {
	...
	entity = plugin_call(key->plugin->key_ops, minimal,);
	aal_memcpy(key, entity, sizeof(*entity));
}
...
errno_t reiser4_key_print(reiser4_key_t *key, aal_stream_t *stream) {
	...
	return plugin_call(key->plugin->key_ops, print, key, stream, 0); 
}
