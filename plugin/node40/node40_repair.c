    ...
    uint16_t start_pos, uint16_t end_pos) 
{
	    ...
	    ih40_get_offset(ih)) 
	{
		...
		ih40_set_offset(ih, ih40_get_offset(ih + 1) + 
		    ih40_get_len(ih + 1));
	}
    }
    ...
    uint16_t end_pos) 
{
    ...
    /* 
	Some lengths between start_pos and end_pos are corrupted or the
	region is not right-bounded by a reliable position. Check if the sum
	of lengths is valid or the set of all offsets are in increasing
	order.
    */
	...
	if ((inval_len && inval_off) || (!inval_len && !inval_off)) {
		...
		aal_exception_error("Node (%llu): the item (%d) offset was "
		    "fixed to (%d).", aal_block_number(node->block), i, 
		    node40_get_offset_at(node->block, i - 1) + 
		    ih40_get_len(ih + 1));
...
/* 
    Checks lengths and offsets of item_heads. If a corrupted region found
    returns 1 and set *start_pos at the first dubious position, *end_pos at the
    ...
    uint16_t *end_pos) 
{
    ...
    *end_pos = node40_count(entity);
	...
	if (offset < sizeof(node40_header_t) || offset > free_end) {
	    blk_t blk = aal_block_number(node->block);
	    if (i == node40_count(entity))
		aal_exception_error("Node (%llu): the start of the free space "
		    "(%llu) is invalid.", blk, offset);
	    else
	...
	if (!(*start_pos)) {
		...
		*start_pos = i;
	    ...
	    /* 
		Left reliable offset should be less then the right reliable 
		at least on 'number of items between them' bytes.
	    */
	    if ((prev_offset + ih40_get_len(ih + 1) == offset) && 
		(node40_get_offset_at(node->block, *start_pos - 1) < 
...
static errno_t node40_item_array_check(reiser4_entity_t *entity) {
    ...
    */
    ...
    while (1) {
	if ((retval = node40_region_find_bad(entity, &start_pos, &end_pos)) 
	    == -1)
	{
...
static errno_t node40_corrupt(reiser4_entity_t *entity, uint16_t options) {
	...
	if (aal_test_bit(i, &options)) {
		...
		node40_set_offset_at(node->block, i -
		    node40_count(entity), INVALID_U16);
	}
    }
