...
static int node_short_isdirty(object_entity_t *entity) {
	aal_assert("umka-2091", entity != NULL);
	return ((node_short_t *)entity)->dirty;
}

static void node_short_mkdirty(object_entity_t *entity) {
	aal_assert("umka-2092", entity != NULL);
	((node_short_t *)entity)->dirty = 1;
}

static void node_short_mkclean(object_entity_t *entity) {
	aal_assert("umka-2093", entity != NULL);
	((node_short_t *)entity)->dirty = 0;
}
#endif

/* Returns node level */
static uint8_t node_short_get_level(object_entity_t *entity) {
	aal_assert("umka-1116", entity != NULL);
	aal_assert("umka-2017", loaded(entity));
	
	return nh_get_level((node_short_t *)entity);
}

#ifndef ENABLE_STAND_ALONE
/* Returns node make stamp */
static uint32_t node_short_get_mstamp(object_entity_t *entity) {
	aal_assert("umka-1127", entity != NULL);
	aal_assert("umka-2018", loaded(entity));
	
	return nh_get_mkfs_id((node_short_t *)entity);
}

/* Returns node flush stamp */
static uint64_t node_short_get_fstamp(object_entity_t *entity) {
	aal_assert("vpf-645", entity != NULL);
	aal_assert("umka-2019", loaded(entity));
	
	return nh_get_flush_id((node_short_t *)entity);
}
#endif

/* Returns item header by pos */
item_header_t *node_short_ih_at(node_short_t *node, uint32_t pos) {
	aal_block_t *block;
	item_header_t *ih;
...
void *node_short_ib_at(node_short_t *node, uint32_t pos) {
	aal_block_t *block;
	item_header_t *ih;
...
uint16_t node_short_free_space_end(node_short_t *node) {
	return aal_block_size(node->block) -
		(nh_get_num_items(node) *
		 sizeof(item_header_t));
}

/* Creates node_short entity on specified device and block number. This can be used
					...
					uint32_t size, blk_t blk)
{
	...
	node->size = size;
	node->device = device;
	node->plugin = &node_short_plugin;

	return (object_entity_t *)node;
}

#ifndef ENABLE_STAND_ALONE
static errno_t node_short_clone(object_entity_t *src_entity,
				object_entity_t *dst_entity)
{
	uint32_t blksize;
	node_short_t *src_node, *dst_node;
	...
	src_node = (node_short_t *)src_entity;
	dst_node = (node_short_t *)dst_entity;
...
static void node_short_move(object_entity_t *entity,
			    blk_t number)
{
	node_short_t *node;
...
static errno_t node_short_form(object_entity_t *entity,
			       uint8_t level)
{
	uint32_t header_size;
	node_short_t *node = (node_short_t *)entity;
	...
	nh_set_magic(node, NODE_SHORT_MAGIC);

	header_size = sizeof(node_short_header_t);
...
static errno_t node_short_load(object_entity_t *entity) {
	node_short_t *node = (node_short_t *)entity;
...
static errno_t node_short_unload(object_entity_t *entity) {
	node_short_t *node = (node_short_t *)entity;
...
static errno_t node_short_close(object_entity_t *entity) {
	node_short_t *node;
				  ...
				  pos_t *pos, key_entity_t *key) 
{
	...
	aal_assert("umka-2344", pos->item <
		   node_short_items(entity));

	body = &(node_short_ih_at((node_short_t *)entity,
				  pos->item)->key);
...
static uint16_t node_short_item_len(object_entity_t *entity, 
				    pos_t *pos)
{
	item_header_t *ih;
	...
	/* Item length is calculated as next item body offset minus current item
	   ...
	   using. */
	ih = node_short_ih_at((node_short_t *)entity, pos->item);

	if (pos->item == (uint32_t)(node_short_items(entity) - 1)) {
		return nh_get_free_space_start((node_short_t *)entity) -
			ih_get_offset(ih);
	}
...
/* Initializes item entity in order to pass it to an item plugin routine. If
   ...
   the unit one. */
static errno_t node_short_get_item(object_entity_t *entity,
				   pos_t *pos, item_entity_t *item)
{
	...
	item->len = node_short_item_len(entity, pos);
...
#ifndef ENABLE_STAND_ALONE
errno_t node_short_item(object_entity_t *entity, pos_t *pos,
			item_entity_t *item)
{
	errno_t res;
	node_short_t *node;
	...
	if ((res = node_short_get_item(entity, pos, item)))
		return res;
...
static uint16_t node_short_space(object_entity_t *entity) {
	aal_assert("vpf-020", entity != NULL);
	aal_assert("umka-2025", loaded(entity));
	
	return nh_get_free_space((node_short_t *)entity);
}
			  ...
			  uint32_t len, uint32_t count)
{
	...
	if (pos->unit == ~0ul) {
		...
		ih = node_short_ih_at(node, pos->item);
		item_len = node_short_item_len(entity, pos);
		       ...
		       uint32_t count)
{
	...
	node_short_t *dst_node;
	node_short_t *src_node;
	item_header_t *ih;
	item_header_t *end;
	void *src, *dst, *body;
				 ...
				 create_hint_t *hint)
{
	...
	if (node_short_item(entity, pos, &item)) {
...
/* Returns node make stamp */
static void node_short_set_mstamp(object_entity_t *entity,
				  uint32_t stamp)
{
	...
	((node_short_t *)entity)->dirty = 1;
	nh_set_mkfs_id((node_short_t *)entity, stamp);
}

/* Returns node flush stamp */
static void node_short_set_fstamp(object_entity_t *entity,
				  uint64_t stamp)
{
	...
	((node_short_t *)entity)->dirty = 1;
	nh_set_flush_id((node_short_t *)entity, stamp);
}

static void node_short_set_level(object_entity_t *entity,
				 uint8_t level)
{
	...
	((node_short_t *)entity)->dirty = 1;
	nh_set_level((node_short_t *)entity, level);
}
...
/* Updating node stamp */
static errno_t node_short_set_stamp(object_entity_t *entity,
				    uint32_t stamp)
{
...
static errno_t node_short_sync(object_entity_t *entity) {
	errno_t res;
	
	aal_assert("umka-1552", entity != NULL);
	aal_assert("umka-2043", loaded(entity));
	
	if ((res = aal_block_write(((node_short_t *)entity)->block)))
		return res;
				...
				uint16_t options) 
{
	...
	node = (node_short_t *)entity;
	level = node_short_get_level(entity);
	...
	aal_stream_format(stream, "NODE (%llu) LEVEL=%u ITEMS=%u "
			  ...
			  node_short_items(entity), node_short_space(entity),
			     ...
			     body_t *key2, void *data)
{
	...
	key1 = &node_short_ih_at((node_short_t *)node, pos)->key;
				  ...
				  pos_t *pos)
{
	...
	switch (aux_bin_search(entity, node_short_items(entity),
...
static bool_t node_short_mergeable(item_entity_t *item1,
				   item_entity_t *item2)
{
			       ...
			       pos_t *dst_pos)
{
	...
	/* Initializing items */
	if (node_short_item(src_entity, src_pos, &src_item))
		return -EINVAL;
	
	if (node_short_item(dst_entity, dst_pos, &dst_item))
		return -EINVAL;
	...
	if ((res = node_short_item(dst_entity, &pos, &dst_item)))
		goto error_free_body;
				...
				shift_hint_t *hint)
{
	...
	src_items = node_short_items(src_entity);
	dst_items = node_short_items(dst_entity);
	...
	if (node_short_item(src_entity, &pos, &src_item))
		return -EINVAL;
	...
	if (dst_items > 0) {
		...
		if (node_short_item(dst_entity, &pos, &dst_item))
			return -EINVAL;
				  ...
				  shift_hint_t *hint)
{
	...
	if (!(src_items = node_short_items(src_entity)))
		return 0;

	space = node_short_space(dst_entity);
		...
		if (flags & SF_UPTIP) {
			...
			if (flags & SF_LEFT) {
				if (hint->pos.item == 0) {
					...
					if (node_short_item(src_entity, &pos, &item))
						return -EINVAL;
				...
				shift_hint_t *hint)
{
	...
	merge.rest = node_short_space(dst_entity);
...
static reiser4_node_ops_t node_short_ops = {
	...
	.load		= node_short_load,
	.close		= node_short_close,
	.unload		= node_short_unload,
	
	.confirm	= node_short_confirm,
	
	.lookup		= node_short_lookup,
	.items		= node_short_items,
	
	.get_key	= node_short_get_key,
	.get_item       = node_short_get_item,
	.get_level	= node_short_get_level,
		
#ifndef ENABLE_STAND_ALONE
	.get_mstamp	= node_short_get_mstamp,
	.get_fstamp     = node_short_get_fstamp,
		
	.move		= node_short_move,
	.clone          = node_short_clone,
	.form		= node_short_form,
	.sync           = node_short_sync,
	.isdirty        = node_short_isdirty,
	.mkdirty        = node_short_mkdirty,
	.mkclean        = node_short_mkclean,
	...
	.space		= node_short_space,
	
	.set_key	= node_short_set_key,
	.set_level      = node_short_set_level,
	.set_mstamp	= node_short_set_mstamp,
	.set_fstamp     = node_short_set_fstamp,
