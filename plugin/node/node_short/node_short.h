...
/* (*(__u32 *)"R4FS") */
#define NODE_SHORT_MAGIC	0x52344653
...
typedef struct node_short node_short_t;

struct node_short_flush {
	d32_t mkfs_id;
	d64_t flush_id;
};

typedef struct node_short_flush node_short_flush_t;

/* Format of node header for node_short */
struct node_short_header {
	...
	/* Node flush stamp */
	node_short_flush_t flush;
};

typedef struct node_short_header node_short_header_t;  

#define	nh(block)                         \
        ((node_short_header_t *)block->data)
...
#define loaded(entity)                 \
        (((node_short_t *)entity)->block != NULL)

extern item_header_t *node_short_ih_at(node_short_t *node,
				       uint32_t pos);

extern uint16_t node_short_free_space_end(node_short_t *node);
extern void *node_short_ib_at(node_short_t *node, uint32_t pos);
