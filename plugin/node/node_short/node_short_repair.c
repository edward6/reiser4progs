...
extern errno_t node_short_item(object_entity_t *entity, pos_t *pos, 
			       item_entity_t *item);
...
static void node_short_set_offset_at(node_short_t *node, int pos,
				     uint16_t offset)
{
...
static errno_t node_short_region_delete(node_short_t *node,
					uint16_t start_pos, 
					uint16_t end_pos) 
{
	uint8_t i;
...
static bool_t node_short_item_count_valid(uint32_t blk_size, uint32_t count) {
	return !( ( blk_size - sizeof(node_short_header_t) ) / 
		  ( sizeof(item_header_t) + MIN_ITEM_LEN ) 
		  < count);
}

static uint32_t node_short_count_estimate(node_short_t *node) {
	...
	if (sizeof(node_short_header_t) + MIN_ITEM_LEN > 
	 ...
	 * blksize. */
	if (nh_get_free_space(node) > blk_size - sizeof(node_short_header_t) - 
	    sizeof(item_header_t) - MIN_ITEM_LEN)
		return 0;
...
static errno_t node_short_item_array_check(node_short_t *node, uint8_t mode) {
	...
	last_pos = 0;
	last_relable = sizeof(node_short_header_t);
	for(i = 0; i <= count; i++) {
...
static errno_t node_short_item_array_find(node_short_t *node, uint8_t mode) {
	...
	for (i = 0; ; i++) {
		...
		if (i) {
			if (offset < sizeof(node_short_header_t) + i * MIN_ITEM_LEN)
				break;	
		} else {
			if (offset != sizeof(node_short_header_t))
				return REPAIR_FATAL;
		}
...
static errno_t node_short_count_check(node_short_t *node, uint8_t mode) {
	uint32_t num;
	blk_t blk;
		    ...
		    copy_hint_t *hint) 
{
	...
	node_short_t *dst_node, *src_node;
	...
	dst_node = (node_short_t *)dst;
	src_node = (node_short_t *)src;
	
	if (hint && hint->src_count == 0)
		return 0;
	
	/* Just a part of src item being copied, gets merged with dst item. */
	if (node_short_item(src, src_pos, &src_item))
		return -EINVAL;
	...
	if (node_short_item(dst, dst_pos, &dst_item))
		return -EINVAL;
