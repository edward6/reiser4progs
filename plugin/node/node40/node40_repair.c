...
extern errno_t node40_shrink(object_entity_t *entity, pos_t *pos, 
...
static bool_t node40_item_count_valid(uint32_t blk_size, uint32_t count) {
	return !( ( blk_size - sizeof(node40_header_t) ) / 
...
static errno_t node40_item_array_check(node40_t *node, uint8_t mode) {
		...
		if (i == 0) {
			if (offset != last_relable) {
				...
				if (mode == REPAIR_REBUILD) {
					ih40_set_offset(node40_ih_at(node, 0), 
							last_relable);
					res |= REPAIR_FIXED;
				} else {
					res |= REPAIR_FATAL;
	...
	if (last_relable != nh40_get_free_space(node)) {
		...
		aal_exception_error("Node (%llu): the free space (%u) is wrong. "
				    "Should be (%u). %s", blk, 
				    nh40_get_free_space(node), last_relable,
...
static errno_t node40_item_array_find(node40_t *node, uint8_t mode) {
	...
	if (offset != nh40_get_free_space_start(node)) {
		aal_exception_error("Node (%llu): Free space start (%u) is wrong. "
				    ...
				    nh40_get_free_space_start(node), offset, 
				    mode == REPAIR_CHECK ? "" : "Fixed.");
	...
	if (offset != nh40_get_free_space_start(node)) {
		aal_exception_error("Node (%llu): Free space (%u) is wrong. "
				    "Should be (%u). %s", blk, 
				    nh40_get_free_space(node), offset, 
				    mode == REPAIR_CHECK ? "" : "Fixed.");
