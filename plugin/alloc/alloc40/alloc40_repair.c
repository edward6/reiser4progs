    ...
    void *data) 
{
    ...
    if (res == -ESTRUCT) {
	/* Rebuild scans everything first, before rebuilding, so mark as much 
	 * as possible to be scanned. Fixable scans only what is reachable 
	 * through the internal tree, so leave it as is o be fixed later. */
