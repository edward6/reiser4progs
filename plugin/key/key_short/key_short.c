...
/* Translates key type from libreiser4 type to key_short one */
static key_short_minor_t key_short_type2minor(key_type_t type) {
	switch (type) {
	case KEY_FILENAME_TYPE:
		return KEY_SHORT_FILENAME_MINOR;
	case KEY_STATDATA_TYPE:
		return KEY_SHORT_STATDATA_MINOR;
	case KEY_ATTRNAME_TYPE:
		return KEY_SHORT_ATTRNAME_MINOR;
	case KEY_ATTRBODY_TYPE:
		return KEY_SHORT_ATTRBODY_MINOR;
	case KEY_FILEBODY_TYPE:
		return KEY_SHORT_FILEBODY_MINOR;
	default:
...
/* Translates key type from key_short to libreiser4 one */
static key_type_t key_short_minor2type(key_short_minor_t minor) {
	switch (minor) {
	case KEY_SHORT_FILENAME_MINOR:
		return KEY_FILENAME_TYPE;
	case KEY_SHORT_STATDATA_MINOR:
		return KEY_STATDATA_TYPE;
	case KEY_SHORT_ATTRNAME_MINOR:
		return KEY_ATTRNAME_TYPE;
	case KEY_SHORT_ATTRBODY_MINOR:
		return KEY_ATTRBODY_TYPE;
	case KEY_SHORT_FILEBODY_MINOR:
...
static key_type_t key_short_get_type(key_entity_t *key) {
	...
	minor = ks_get_minor((key_short_t *)key->body);
	return key_short_minor2type(minor);
}
...
static int key_short_large(void) {
	return 0;
}
				     ...
				     char *name) 
{
	...
	type = key_short_minor2type(
		KEY_SHORT_FILENAME_MINOR);
...
static errno_t key_short_valid(key_entity_t *key) {
	...
	if (minor == KEY_SHORT_FILENAME_MINOR && band == 1)
		return 0;

	return -EINVAL;
}

/* Key minor names. They are used key_short_print() function */
static const char *const minor_names[] = {
	"NAME", "SD", "AN", "AB", "FB", "?"
};

/* Translates passed minor into corresponding name */
const char *key_short_minor2name(key_short_minor_t type) {
	if (type > KEY_SHORT_LAST_MINOR)
		type = KEY_SHORT_LAST_MINOR;
			...
			uint16_t options) 
{
	...
	aal_stream_format(stream, "[ key_short %llx:%x:%llx:%llx %s ]",
			  ...
			  key_short_minor2name(key_short_get_type(key)));
