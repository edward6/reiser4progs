...
/* Translates key type from libreiser4 type to key_large one */
static key_large_minor_t key_large_type2minor(key_type_t type) {
	switch (type) {
	case KEY_FILENAME_TYPE:
		return KEY_LARGE_FILENAME_MINOR;
	case KEY_STATDATA_TYPE:
		return KEY_LARGE_STATDATA_MINOR;
	case KEY_ATTRNAME_TYPE:
		return KEY_LARGE_ATTRNAME_MINOR;
	case KEY_ATTRBODY_TYPE:
		return KEY_LARGE_ATTRBODY_MINOR;
	case KEY_FILEBODY_TYPE:
		return KEY_LARGE_FILEBODY_MINOR;
	default:
...
/* Translates key type from key_large to libreiser4 one */
static key_type_t key_large_minor2type(key_large_minor_t minor) {
	switch (minor) {
	case KEY_LARGE_FILENAME_MINOR:
		return KEY_FILENAME_TYPE;
	case KEY_LARGE_STATDATA_MINOR:
		return KEY_STATDATA_TYPE;
	case KEY_LARGE_ATTRNAME_MINOR:
		return KEY_ATTRNAME_TYPE;
	case KEY_LARGE_ATTRBODY_MINOR:
		return KEY_ATTRBODY_TYPE;
	case KEY_LARGE_FILEBODY_MINOR:
...
static key_type_t key_large_get_type(key_entity_t *key) {
	...
	minor = kl_get_minor((key_large_t *)key->body);
	return key_large_minor2type(minor);
}
...
static int key_large_large(void) {
	return 1;
}
				     ...
				     char *name) 
{
	...
	type = key_large_minor2type(
		KEY_LARGE_FILENAME_MINOR);
...
static errno_t key_large_valid(key_entity_t *key) {
	...
	if (minor == KEY_LARGE_FILENAME_MINOR && band == 1)
		return 0;

	return -EINVAL;
}

/* Key minor names. They are used key_large_print() function */
static const char *const minor_names[] = {
	"NAME", "SD", "AN", "AB", "FB", "?"
};

/* Translates passed minor into corresponding name */
const char *key_large_minor2name(key_large_minor_t type) {
	
	if (type > KEY_LARGE_LAST_MINOR)
		type = KEY_LARGE_LAST_MINOR;
			...
			uint16_t options) 
{
	...
	name = key_large_minor2name(key_large_get_type(key));
	
	aal_stream_format(stream, "[ key_large %llx:%x:%llx:%llx:%llx %s ]",
