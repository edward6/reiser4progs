...
typedef struct journal40_footer {
...
typedef struct journal40_tx_header {
...
typedef struct journal40_lr_header {
	char magic[LGR_MAGIC_SIZE];
	d64_t lh_id;
	d32_t lh_total;
	d32_t lh_serial;
	d64_t lh_next_block;
} journal40_lr_header_t;
...
typedef struct journal40_lr_entry {
	d64_t le_original;
	d64_t le_wandered;
} journal40_lr_entry_t;

typedef enum journal40_block {
	JB_INV = 0x0,
	JB_TXH = 0x1,
	JB_LGR = 0x2,
	JB_WAN = 0x3,
