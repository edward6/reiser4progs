...
extern int32_t cde_large_remove(place_t *item, uint32_t pos, 
				uint32_t count);

extern errno_t cde_large_get_key(place_t *place, uint32_t pos, 
				 key_entity_t *key);
...
extern errno_t cde_large_maxposs_key(place_t *place, key_entity_t *key);

extern errno_t cde_large_rep(place_t *dst_place, uint32_t dst_pos,
			     place_t *src_place, uint32_t src_pos,
			     uint32_t count);

extern int32_t cde_large_expand(place_t *place, uint32_t pos,
				uint32_t count, uint32_t len);

/* Check the i-th offset of the unit body within the item. */
static errno_t cde_large_offset_check(place_t *place, uint32_t pos) {
...
static uint32_t cde_large_count_estimate(place_t *place, uint32_t pos) {
	cde_large_t *de = cde_large_body(place);
...
static errno_t cde_large_pair_offsets_check(place_t *place, 
					    uint32_t start_pos, 
					    uint32_t end_pos) 
{    
	cde_large_t *de = cde_large_body(place);
...
static uint8_t cde_large_short_entry_detect(place_t *place, 
					    uint32_t start_pos, 
					    uint32_t length, 
					    uint8_t mode)
{
...
/* Returns amount of entries detected. */
static uint8_t cde_large_long_entry_detect(place_t *place, 
					   uint32_t start_pos, 
					   uint32_t length, 
					   uint8_t mode)
{
	...
	while (l_limit < r_limit) {
		offset = cde_large_name_end(place->body, 
...
static inline uint8_t cde_large_entry_detect(place_t *place, uint32_t start_pos,
...
static errno_t cde_large_offsets_range_check(place_t *place, 
					     ...
					     uint8_t mode) 
{
	...
	for (i = 0; i < flags->count; i++) {
		...
		if (to_compare == MAX_UINT32) {
			if ((i == 0) && (cde_large_count_estimate(place, i) == 
					 de_get_units(de)))
			{
		...
		for (j = to_compare; j < i; j++) {
			...
			if (!cde_large_pair_offsets_check(place, j, i)) {
				/* Pair looks ok. Try to recover it. */
				if (cde_large_entry_detect(place, j, i, mode)) {
					uint32_t limit;
					...
					/* It's possible to decrease the count 
					   when first R found. */
					limit = cde_large_count_estimate(place, j);
...
static errno_t cde_large_filter(place_t *place, struct entry_flags *flags,
				uint8_t mode)
{
	cde_large_t *de = cde_large_body(place);
	...
	if (OFFSET(de, last) + sizeof(objid_t) == place->len) {
		...
		offset = cde_large_name_end(place->body, OFFSET(de, last) + 
	...
	e_count = cde_large_count_estimate(place, i);
	...
	if (i) {
		...
		aal_exception_error("Node %llu, item %u: entries [%u..%u] look "
				    " corrupted. %s", place->block->nr, 
				    place->pos.item, 0, i - 1, mode == RM_BUILD 
				    ? "Removed." : "");
	...
	/* Units before @i and after @count were handled, do not care about 
	   them anymore. Handle all not relable units between them. */
	last = MAX_UINT32;
	for (; i < flags->count; i++) {
		if (last == MAX_UINT32) {
		...
		if (aal_test_bit(flags->elem + i, R)) {
			...
			if (cde_large_remove(place, last, i - last) < 0) {
				aal_exception_error("Node %llu, item %u: remove"
						    " of unit (%u), count (%u) "
						    "failed.", place->block->nr, 
						    place->pos.item, last, 
...
errno_t cde_large_estimate_copy(place_t *dst, uint32_t dst_pos,
...
errno_t cde_large_copy(place_t *dst, uint32_t dst_pos, 
			place_t *src, uint32_t src_pos, 
			copy_hint_t *hint)
{
...
#if 0
{
	if ((res = cde_large_count_check(place)))
		return res;
...
static errno_t cde_large_count_check(place_t *place, 
...
static errno_t cde_large_bad_range_check(place_t *place, 
					 uint32_t start_pos, 
					 uint32_t end_pos, 
					 repair_plugin_info_t *info) 
{
	...
	start_again:
		...
		for (i = 1; i < end - start; i++) {
			offset = cde_large_name_end(place->body, l_limit, r_limit);
