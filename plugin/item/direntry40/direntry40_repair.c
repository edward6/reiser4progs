					    ...
					    uint8_t mode)
{
	...
	while (l_limit < r_limit) {
		    ...
		    l_limit != OFFSET(de, start_pos + count)) 
		{
			...
			if (mode == REPAIR_REBUILD)
				en40_set_offset(&de->entry[start_pos + count], 
						l_limit);
		}
	}
...
static inline uint8_t direntry40_entry_detect(item_entity_t *item, uint32_t start_pos,
				 ...
				 uint8_t mode)
{
	...
	if (e_count != de40_get_units(de)) {
		aal_exception_error("Node %llu, item %u: unit count (%u) is not "
				    ...
				    item->pos.item, de40_get_units(de), e_count, 
	...
	for (; i < flags->count; i++) {
		if (last == ~0ul) {
			...
			if (aal_test_bit(flags->elem + i, R)) {
				...
				if (mode == REPAIR_REBUILD) {
					if (direntry40_remove(item, last, 
							      i - last) < 0) 
					{
...
#if 0
{
	...
	for (i = 1; i <= de40_get_units(de); i++) {
		...
		offset = (i == de40_get_units(de) ? item->len : 
			  en40_get_offset(&de->entry[i]));

		/* Check the offset. */
		if ((offset - ENTRY_MIN_LEN * interval < 
		     en40_get_offset(&de->entry[i - 1])) || 
		    (offset + ENTRY_MIN_LEN * (de40_get_units(de) - i) > 
		     (int)item->len))
			{
				      ...
				      repair_plugin_info_t *info) 
{
	...
	if (de40_min_length(de40_get_units(de)) > item->len) {
		...
		aal_exception_error("Node %llu, item %u: unit count (%u) is "
				    ...
				    item->pos, de40_get_units(de), count);
		    ...
		    (count != de40_get_units(de))) 
		{
			...
			aal_exception_error("Node %llu, item %u: wrong offset "
					    ...
					    item->context.blk, item->pos,
					    de->entry[0].offset, 
					    de40_get_units(de) * sizeof(entry40_t) + 
					    sizeof(direntry40_t));
...
static errno_t direntry40_bad_range_check(item_entity_t *item, 
					  uint32_t start_pos, 
					  uint32_t end_pos, 
					  repair_plugin_info_t *info) 
{
